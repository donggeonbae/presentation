...
- Current status: presentation poster archive refreshed from the figure repo specs; 225k, 230k, and 235k full-test evaluations are complete, while the 330k full-test evaluation remains pending.
...
| Takeaway | Set expectations | Ours beats physics-only; 225k is current best while final 5epoch evaluation is pending | concise table and convergence note |
...
Current full-test evidence uses the 225k-step v15 checkpoint. Ours improves foreground delta3 from 0.816 for the physics-only focus baseline to 0.938, and reduces foreground MAE from 0.214 to 0.0536. The 230k and 235k full-test evaluations are weaker, and the 330k final evaluation is queued behind the active 5epoch training run. Final poster table values should be locked after the remaining 330k full-test JSON completes.

### Conclusion

The preliminary evidence supports the main claim that depth-dependent deconvolution features are useful physics cues for latent diffusion depth estimation. The final conclusion should be updated after the full 6,000-sample evaluation of the converged 330k checkpoint.
...
This project estimates depth from lensless measurements using a 42-plane PSF stack. The key point is that different deconvolution planes focus different depths, so we treat the deconvolution stack as physics evidence inside a latent diffusion model. Ours v15 is the final diffusion model family: it uses latent denoising plus deconvolution/focus posterior guidance. Preliminary full-test results beat the physics-only focus baseline, and 225k is the current best completed checkpoint while final 5epoch evaluation is pending.

### 2-Minute Version

Lensless measurements mix spatial content and depth-dependent blur. If we deconvolve the same measurement with different PSF planes, different scene regions become sharper depending on depth. A plain pipeline would feed that deconvolution stack into a supervised UNet, but our goal is to put the physics into the diffusion process. Ours v15 encodes depth into latent space, denoises the latent depth, and conditions/guides reverse diffusion using PSF-stack deconvolution, focus probabilities, and auxiliary focus cues. In the current best full-test result, Ours improves foreground delta3 from 0.816 for physics-only focus scoring to 0.938, with foreground MAE dropping from 0.214 to 0.0536. The 230k and 235k full evaluations are weaker; we are still waiting for the final 330k evaluation before locking the reported model.
...
| Are the final numbers done? | Not yet. Current paper/poster numbers use the 225k full-test result; the 330k evaluation is queued after training. |
