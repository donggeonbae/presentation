...
- Current status: presentation poster archive refreshed from the figure repo specs; the 225k full-test evaluation is complete, while 230k/235k and 330k full-test evaluations remain pending.
...
| Takeaway | Set expectations | Ours beats physics-only; final 5epoch full-test evaluation is pending | concise table and convergence note |
...
Current full-test evidence uses the 225k-step v15 checkpoint. Ours improves foreground delta3 from 0.816 for the physics-only focus baseline to 0.938, and reduces foreground MAE from 0.214 to 0.0536. The 230k and 235k evaluations are weaker so far, and the 330k final evaluation is queued behind the active 5epoch training run. Final poster table values should be locked after the remaining full-test JSON files complete.

### Conclusion

The preliminary evidence supports the main claim that depth-dependent deconvolution features are useful physics cues for latent diffusion depth estimation. The final conclusion should be updated after the full 6,000-sample evaluations of the 225k and converged 330k checkpoints.
...
| Are the final numbers done? | Not yet. Current paper/poster numbers are preliminary 195k full-test results; 225k evaluation is running and 330k evaluation is queued after training. |
